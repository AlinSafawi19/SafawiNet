...
export default function MyAccountPage() {
    ...
    if (prevValues.current) {
        ...
        if (JSON.stringify(prevValues.current.user) !== JSON.stringify(currentValues.user)) {
            changes.push('user changed');
        }
        if (prevValues.current.isLoading !== currentValues.isLoading) {
            changes.push('isLoading changed');
        }
        if (prevValues.current.locale !== currentValues.locale) {
            changes.push('locale changed');
        }
        if (prevValues.current.tFunction !== currentValues.tFunction) {
            changes.push('tFunction changed');
        }
                ...
                <div className="account max-w-4xl mx-auto">
                    ...
                    <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                        {[
                            {
                                id: 'login-security',
                                title: t('account.loginSecurity.title'),
                                description: t('account.loginSecurityDesc'),
                            ...
                            },
                            {
                                ...
                                description: t('account.loyaltyDesc'),
                            ...
                            >
                                <div className="flex flex-col h-full">
                                    {/* Icon and Content */}
                                    <div className={`flex items-start mb-4 ${locale === 'ar' ? 'flex-row-reverse space-x-reverse space-x-4' : 'space-x-4'}`}>
                                        ...
                                        <div className="flex-1">
                                            <h3 className={`text-lg font-semibold text-gray-900 mb-2 ${locale === 'ar' ? 'text-right' : 'text-left'}`}>
                                                {section.title}
                                            </h3>
                                            <p className={`text-sm text-gray-600 ${locale === 'ar' ? 'text-right' : 'text-left'}`}>
                                                {section.description}
                                            </p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="mt-auto">
                                        <Link
                                            href={section.href}
                                            className={`inline-flex items-center justify-center w-full px-4 py-2 text-sm font-medium text-white bg-black border border-transparent rounded-md hover:shadow-lg hover:shadow-purple-500/25 transition-all duration-300 focus:outline-none ${locale === 'ar' ? 'flex-row-reverse' : ''
                                                }`}
                                        >
                                            <span className={locale === 'ar' ? 'text-right' : 'text-left'}>{t('common.clickHere')}</span>
                                            <svg className={`w-4 h-4 ${locale === 'ar' ? 'mr-2' : 'ml-2'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
